...
def noisy_best_replacement(population, obj_fn, replaced_count):
    ...
    for i in range(replaced_count):
        ...
        population.overwrite(-i-1, copied_best + 2.0*nprd.random(len(copied_best)) - 1.0)
...
def random_replacement(population, obj_fn, replaced_count):
    ...
    for i in range(replaced_count):
        population.overwrite(-i-1, minimums + nprd.random(size=minimums.size) * (maximums - minimums))
